...
> 5. **Level loose-ends** (`PointSourcePolygon.Config.level` + `ClockwiseSweepPolygon` internals).
> 6. **`canvas/vfx/`** (~21 files) — **deprioritized per the scope-priority decision; do only if explicitly requested** (no v13 analog, no consumers).
>
