...
| **5** | Canvas: verify + fill + add (ex-vfx)                                                                  | **ACTIVE** — 5.1–5.6e done & CI-green (all 5 shape leaf placeables + doc `shape`/`_shape` props); **5.6f next** (ShapeLayerMixin + 5 leaf layers), then 5.6g + 5.7 | [phase-5](migration-v14-phase-5.md) |
...
>
> - **Phase 5.6f** (canvas — **5.6e just closed**) — `layers/mixins/shapes` (**ShapeLayerMixin**) + the 5 `ShapeLayerMixin(PlaceablesLayer)` leaf layers (drawings/lighting/sounds/regions/tiles, reassigned from 5.5). **⚠ This hits the same tsgo circular-alias bug as the shape placeables — give it a `ShapeLayerPlaceablesLayer<…>`-style intermediate (mirror the `ShapePlaceableObject` fix in `placeables/mixins/shapes.d.mts`).** Also resolve the `paletteClass` FIXMEs once 5.6g lands. Read [phase-5](migration-v14-phase-5.md) (Batch 5.6 row + the 5.6e cycle finding).
...
>
> ```
> Continue the v14 migration (Batch 5.6f — ShapeLayerMixin + 5 leaf layers)
> ```
>
> ```
