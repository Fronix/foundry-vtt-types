...
| **5** | Canvas: verify + fill + add (ex-vfx)                                                                  | **ACTIVE** — 5.1–5.6f done & CI-green (ShapeLayerMixin + 5 reparented leaf layers); **5.6g next** (palette apps), then 5.7 | [phase-5](migration-v14-phase-5.md) |
...
>
> - **Phase 5.6g** (canvas — **5.6f just closed**) — author the palette app classes in `applications/sheets/palette/` (`DrawingPalette`/`AmbientLightPalette`/`AmbientSoundPalette`/`TilePalette`/`RegionPalette`/`NotePalette`/`WallPalette`), then resolve the `static paletteClass` FIXMEs across the leaf layers + the `paletteCreateData` getter return in `placeables-layer.d.mts`. Read [phase-5](migration-v14-phase-5.md) (the 5.6g deferral row). After 5.6g comes **5.7** (interaction/animation/top-level/extensions/workers).
...
>
> ```
> Continue the v14 migration (Batch 5.6g — palette app classes)
> ```
>
> ```
