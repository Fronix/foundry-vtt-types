...
| **7** | Greenfield: vfx, region-behaviors, **Scene Levels**, 4 Tier-B giants, MeasuredTemplate, shapes barrel | **In progress** — Scene Levels largely landed: `Level` doc + `SceneLevelsSetField` + `TEXTURE` category + Scene embedding (`6632620a0`); per-placeable `levels`/`level` fields (`ed5eb4a0d`); `object`-typed Level FIXMEs tightened (`e244d7bfa`). CI green. Remaining: 4 Tier-B giants, vfx, region-behaviors, MeasuredTemplate reconcile, shapes barrel, polygon `Config.level`. **Inherits deferrals.** | [phase-7](migration-v14-phase-7.md) |
...
>
> **Phase 7 is IN PROGRESS (2026-05-28) — CI green. Scene Levels is largely landed** across three commits: the `Level` document + `SceneLevelsSetField` + `TEXTURE` file category + Scene embedding (`6632620a0`); the per-placeable `levels`/`level` schema fields on tile/note/drawing/ambient-light/ambient-sound/region/wall/token (`ed5eb4a0d`); and the `object`-typed Level FIXMEs tightened to `Level.Implementation` across canvas + scene-config (`e244d7bfa`).
>
> **Next, in priority order:**
>
> 1. **The 4 Tier-B document giants** — the highest remaining leverage. `active-effect` is independent (needs `ActiveEffectRegistry` + CONFIG additions + duration rework + the change-application instance→static move). `scene`/`region`/`token` now have a real `Level` to type against but still need `common/data/_module` `operators` re-export (`operators.mjs`) + the token-movement `_types`. Author each prerequisite-first, then member-diff, **one giant per session** (each is 1.1k–4k lines). See the phase-7 "Tier-B giant" inherited-deferral row for the per-giant blocker list.
> 2. **Remaining Level loose-ends:** `PointSourcePolygon.Config.level` + `ClockwiseSweepPolygon` level internals (source-polygon not yet touched); `CanvasOcclusionMask#occludedSurfaces` `RegionSurface` (rides with the region giant).
> 3. **Greenfield:** `data/region-behaviors/` (~4 files), then `canvas/vfx/` (~21 files, deprioritized).
...
>
> ```
> Continue the v14 migration (Phase 7 — start the Tier-B document giants: author the active-effect prerequisites (ActiveEffectRegistry, CONFIG additions, duration) then its member-diff; OR author operators.mjs + do the scene giant)
> ```
>
