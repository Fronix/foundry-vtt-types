...
| **8** | Cleanup: remove `removed in v14` deprecations, bump to `14.x`, backfill tests                         | **DONE (pragmatic high-value scope) — CI green** (commit `13dda9913`). `removed in v14` pass done **non-breaking**: removed category (b) accuracy wins (members v14 genuinely dropped), kept + reworded category (a) migration aids (TS-only alias renames) → `removed in a future version`. Version bump + inherited embedded-collection stubs done. Non-gating remainders documented: `_onXDocuments` static-method structural removal (document.d.mts boundary, sign-off), ~210 `until v14` reword, `TypedObjectField` create-optionality, member-order parity. | [phase-8](migration-v14-phase-8.md) |
...
>
> **Phase 8 is DONE (pragmatic high-value scope) — CI green (commit `13dda9913`). All 8 phases are now closed — the v14 migration's pragmatic high-value scope is complete.**
>
> **What P8 did (2026-05-28):** the `removed in v14` deprecation pass, done **non-breaking** by maintainer decision. Investigation showed the feared bulk — the database-operation alias renames (`CreateDocumentsOperation`→`CreateOperation`, etc.) — are **TS-only convenience aliases, not runtime members**, so keeping them costs nothing in v14 accuracy and eases migration. Decision: **remove category (b) accuracy wins** (members the v14 runtime genuinely dropped / hard-privatised — each verified absent against the v14.363.0 source) and **keep + reword category (a) migration aids** (`will be removed in v14` → `removed in a future version`). Also: version already at `14.363.0`; the inherited `embedded-collection`/`embedded-collection-delta` stub deferrals actioned. CI green (typecheck, `tsc --eOPT false`, raw tsgo, eslint, prettier, test-types). Full remove/keep inventory in [phase-8](migration-v14-phase-8.md).
>
> **Explicitly non-gating remainders (documented in [phase-8](migration-v14-phase-8.md), none consumer-blocking):**
>
> 1. **`_onCreateDocuments`/`_onUpdateDocuments`/`_onDeleteDocuments` static methods** (3 base in `document.d.mts` typed `never` + 35 leaf overrides) — gone from v14 runtime but deprecated `protected static` migration aids; structural removal cascades into the `document.d.mts` **boundary** type machinery (the `Extract`-branch `OnCreateDocumentsOperation` derivation, `// TODO: remove ... in v14`) → needs **explicit human sign-off**, kept for a coordinated boundary pass.
> 2. **~210 `until v14` markers** — a separate deprecation-window bucket; reword opportunistically (`until v14` → `until a future version`). Includes the item-1 method notes.
> 3. **`TypedObjectField` create-optionality** — give it `SchemaField`'s implicit-`{}`-initial, then drop the `token.detectionModes` `{ initial: Record<string, never> }` workaround. `fields.d.mts` ergonomic enhancement, not phase-gated.
> 4. **Member-order parity** on `scene`/`active-effect` — out of scope per the [scope-priority decision](#scope-priority).
>
> **Assessment:** the multi-phase v14 migration is complete to its agreed pragmatic high-value scope and CI-green. The remainders are a sign-off-gated boundary refactor, opportunistic comment tidy, an optional `fields.d.mts` enhancement, and cosmetic parity — all explicitly deprioritised. The package models v14 accurately for what consumers touch. Remaining work would be the `v14`→`main` fast-forward merge (fork-only, conflict-free per branch strategy) when the maintainer is ready.
>
> **Kickoff message** (paste verbatim):
>
> ```
> The v14 migration is complete to its pragmatic high-value scope (all 8 phases done, CI-green). Phase 8's `removed in v14` pass was done non-breaking: accuracy wins removed, migration-aid aliases kept + reworded to "a future version" (see migration-v14-phase-8.md). Four explicitly non-gating remainders are documented (the _onXDocuments static-method structural removal needs a signed-off document.d.mts boundary pass; the ~210 `until v14` reword; the TypedObjectField create-optionality enhancement; member-order parity). If you want to ship, the next step is the v14→main fast-forward merge (fork-only). Otherwise pick up any documented remainder — confirm sign-off before touching the document.d.mts boundary.
> ```
>
