...
>
> **No giants remain.** Also done this session: token `Position` depth/level drift fix; the **`canvas/vfx/` subsystem (all 27 files)**; the **Level/surfaceExposure source-polygon loose-ends**; and the **MeasuredTemplate deprecation surface** (`@remarks` prose + `_fromRegion`). **Only three items remain, all gated on human input:**
>
> 1. **MeasuredTemplate structural un-embed — BLOCKED, needs human-reviewed architecture.** v14 removes `MeasuredTemplate` from `ALL_DOCUMENT_TYPES` (it's in neither `PRIMARY` nor `EMBEDDED`) while it stays a deprecated `Document` subclass with `CONFIG.MeasuredTemplate`. The repo can't represent a "deprecated, non-registered `Document` subclass" without redesigning `document.d.mts`'s `DocumentName` constraint + `documentConfiguration.d.mts` + `src/configuration/` + the canvas `AnyCanvasDocument`/Levels constraints. **Do NOT attempt piecemeal** — it cascades destructively (verified). **The client-data shapes-barrel unification + polygon-tree file-move is the SAME class of blocker** (client shape classes would shadow the common `BaseShapeData` subclasses, breaking `RegionDocument#shapes`). Both need a dedicated human-designed change.
> 2. **P8 cleanup** (this is Phase 8, not Phase 7) — `package.json` → `14.363.0` **DONE 2026-05-28** (pulled forward). The remaining `removed in v14` deprecation-alias prune (~892 occ across 78 files) is a **breaking change** touching the `document.d.mts` boundary — left for a deliberate coordinated P8 pass with sign-off.
> 3. **Member-order polish passes** — `scene`/`active-effect` have some pre-existing members in v13 order (region/token were added in source order this session). Cosmetic (reviewer-diff convenience only); the scope-priority decision deprioritizes this kind of full-parity polish. High churn across giant files for no functional gain.
>
> **Assessment:** every high-value and safely-automatable Phase 7 item is done and CI-green — the 4 Tier-B giants, Scene Levels, region-behaviors, operators, the MeasuredTemplate deprecation surface, the full vfx subsystem, and the Level/surface loose-ends. The three remaining items are architecturally-blocked (1), release/breaking decisions (2), or cosmetic polish the maintainer deprioritized (3). This is consistent with the [scope-priority decision](#scope-priority): pragmatic high-value subset, not full parity. Closing the rest needs human design/sign-off, not autonomous grinding.
>
> **Kickoff message** (paste verbatim):
>
> ```
> Continue the v14 migration (Phase 7 tail). Everything safely-automatable is done and CI-green: 4 Tier-B giants, Scene Levels, region-behaviors, operators, MeasuredTemplate deprecation surface, the full canvas/vfx subsystem (27 files), and the source-polygon Level/surfaceExposure loose-ends. Three items remain, all gated: (1) the MeasuredTemplate structural un-embed AND the client-data shapes-barrel unification are both architecturally blocked — they remove a type from ALL_DOCUMENT_TYPES / shadow common shape classes, requiring a redesign of document.d.mts/documentConfiguration/src/configuration + canvas AnyCanvasDocument constraints (human design needed; see migration-v14-phase-7.md). (2) P8 version bump (release decision) + pruning 141 `removed in v14` aliases (breaking) need sign-off. (3) member-order polish on scene/active-effect is cosmetic and deprioritized. Decide which, if any, to pursue.
> ```
>
