...
> 6. **`canvas/vfx/`** (27 files / ~2984 lines) — **keep deprioritized.** Foundry's own `vfx/_module` marks the entire framework **EXPERIMENTAL and non-stable** ("classes, functions, and configuration are likely to change over coming releases"). Authoring types now models something Foundry says will change — low value, no consumers. Defer to a dedicated effort once Foundry stabilizes it.
>
