...
>
> **No giants remain.** The remaining Phase 7 items are smaller polish/deprioritized work — pick the highest-value:
>
> 1. **MeasuredTemplate reconciliation** — the `@deprecated since v14` class-level cascade for `measured-template.d.mts` + canvas `TemplateLayer`/`MeasuredTemplate` placeable + the Scene un-embed (see the MeasuredTemplate deferral row in phase-7). Highest remaining consumer-facing value.
> 2. **P8 cleanup pulled forward** — bump `package.json` → `14.x`; prune the `removed in v14` deprecations (incl. `toggleActiveEffect`). Cheap accuracy wins.
> 3. **Member-order polish passes** — `scene`, `active-effect`, `region`, `token` signatures are correct but some pre-existing members sit in v13 order.
> 4. **Client-data shapes-barrel unification + polygon-tree file-move** (`region-shapes/polygon-tree`→`data/polygon-tree`, `RegionPolygonTree`→`PolygonTree`) + `CanvasOcclusionMask#occludedSurfaces` (now that `RegionSurface` exists).
> 5. **Token `Position` depth/level drift** — add v14's `depth`/`level` to `TokenDocument.Position` and ripple through the movement-waypoint types.
> 6. **Level loose-ends** (`PointSourcePolygon.Config.level` + `ClockwiseSweepPolygon` internals).
> 7. **`canvas/vfx/`** (~21 files) — **deprioritized; do only if explicitly requested** (no v13 analog, no consumers).
>
> **Kickoff message** (paste verbatim):
>
> ```
> Continue the v14 migration (Phase 7 — all 4 Tier-B giants are done. Next highest-value item: the MeasuredTemplate reconciliation — apply the @deprecated since v14 class-level annotation to client/documents/measured-template.d.mts + canvas TemplateLayer + the MeasuredTemplate placeable, and the Scene un-embed, landing the no-deprecated/missing-type cascade coherently. See the MeasuredTemplate deferral row in migration-v14-phase-7.md. Then pull forward the cheap P8 cleanup (bump package.json to 14.x, prune `removed in v14`).)
> ```
>
