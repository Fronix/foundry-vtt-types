...
| **7** | Greenfield: vfx, region-behaviors, **Scene Levels**, 4 Tier-B giants, MeasuredTemplate, shapes barrel | **In progress** — DONE: Scene Levels (`6632620a0`/`ed5eb4a0d`/`e244d7bfa`), `common/data/operators` (`0e533917e`), all 4 region-behaviors, **active-effect giant**, **scene giant** (substantially), **region giant** (`RegionSurface` prereq + full member-diff, this batch). CI green. **Remaining Tier-B giant: token** (needs token-movement `_types`); plus vfx (~21, deprioritized), MeasuredTemplate reconcile, shapes barrel + polygon-tree move, polygon `Config.level`, member-order polish passes. **Inherits deferrals.** | [phase-7](migration-v14-phase-7.md) |
...
>
> **Phase 7 is IN PROGRESS (2026-05-28) — CI green.** This batch closed the **region Tier-B giant**: authored the `RegionSurface` prerequisite (`RegionDocument.Surface` + `RegionSurface` alias in `client/documents/_types.d.mts`) and member-diffed `client/documents/region.d.mts` — added `isSingleShape`/`clipperPolyTree`/`area`, `clampElevation`, `_createClipperPolyTree`, the shape-constraint family (`updateShapeConstraints`/`_computeShapeConstraints`/`_computeShapeConstraint`→`PointSourcePolygon`), `_onGridChange`/`_clearPolygonTree`/`_onPolygonTreeChange`, `removeShapeDialog`, static `createTokenEmanation`, `teleportToken`(+options)/`teleportTokens`/`spawnTokens`, and the Deprecations block (`regionShapes` deprecated + live `_regionShapes`). Modeled `attachment.token` id→document resolution via the field InitializedType override. Dropped the stale `_updateTokens`. Convention-omitted the signature-unchanged inheritDoc overrides. Added `region.test-d.ts`. **NOT done (still deferred):** the polygon-tree file-move + client-data shapes-barrel unification (kept existing `RegionPolygonTree`/`regionShapes`, structurally identical) + `CanvasOcclusionMask#occludedSurfaces` FIXME.
>
> **Next — the last Tier-B giant:**
>
> 1. **`token` giant (4009)** — author the token-movement subsystem (~15 `TokenMovement*` `_types` + `TokenConstrainMovementPathOptions`), then member-diff `client/documents/token.d.mts`; this also unblocks the deferred Token-**placeable** drag/movement surface, the canvas `tokens.d.mts` movement FIXMEs (Batch 5.5b), and `ModifyMovementCost._getTerrainEffects`. (`Level`/`operators`/`DataFieldOperator` prereqs already met.)
>
> **Then the smaller remaining P7 items:** member-order polish passes (`scene`, `active-effect`, `region` — signatures done, some pre-existing members still in v13 order); the client-data shapes-barrel unification + polygon-tree move (rides with region, now deferred) + `CanvasOcclusionMask#occludedSurfaces`; Level loose-ends (`PointSourcePolygon.Config.level` + `ClockwiseSweepPolygon` internals); `canvas/vfx/` (~21, deprioritized); MeasuredTemplate reconciliation (the `@deprecated`-class cascade) + **P8 cleanup** (bump `package.json` → `14.x`, prune `removed in v14`).
>
> **Kickoff message** (paste verbatim):
>
> ```
> Continue the v14 migration (Phase 7 — active-effect, scene, and region Tier-B giants are done; now do the LAST giant, token: author the token-movement subsystem (~15 TokenMovement* _types + TokenConstrainMovementPathOptions) in client/documents/_types.d.mts first, then member-diff client/documents/token.d.mts against the v14 source. This also unblocks the Token placeable drag/movement surface and the canvas tokens.d.mts movement FIXMEs. One giant per session.)
> ```
>
