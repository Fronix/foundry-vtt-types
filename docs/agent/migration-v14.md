...
| **7** | Greenfield: vfx, region-behaviors, **Scene Levels**, 4 Tier-B giants, MeasuredTemplate, shapes barrel | **In progress** — DONE: Scene Levels (`6632620a0`/`ed5eb4a0d`/`e244d7bfa`), `common/data/operators` (`0e533917e`), all 4 region-behaviors, **active-effect giant** (change-application instance→static + duration rework + deprecation shims, this batch), **scene giant** (substantially). CI green. **Remaining Tier-B giants: region** (needs `RegionSurface`), **token** (needs token-movement `_types`); plus vfx (~21, deprioritized), MeasuredTemplate reconcile, shapes barrel, polygon `Config.level`. **Inherits deferrals.** | [phase-7](migration-v14-phase-7.md) |
...
>
> **Phase 7 is IN PROGRESS (2026-05-28) — CI green.** This batch closed the **active-effect Tier-B giant's risky core**: the change-application family moved instance→static (`applyChange`/`applyChangeField`/`_applyChangeUnguided`/`_applyChange{Add,Subtract,Multiply,Override,Upgrade,Custom}`/`_replaceDataRefs`), the v13 instance methods became `@deprecated since v14, until v16` shims, the `ActiveEffectDuration` rework landed (`_prepareTimeBasedDuration`/`_prepareCombatBasedDuration`; removed v13 `_requiresDurationUpdate`/`_getCombatTime`/`_getDurationLabel`), `ChangeData` migrated off `mode`→`type`/`phase`, the loose `CONFIG.ActiveEffectChangeTypeConfig` `handler`/`render` callbacks were tightened to `ActiveEffect.ChangeHandler`/`ChangeRenderer`, and the missing `actor`/`item`/`thumbnail`/`CHANGE_PHASES`/`CHANGE_TYPES`/`EXPIRY_EVENTS`/`getEffectStart` members were added. All 5 gates green.
>
> **Next, in priority order — two Tier-B giants remain:**
>
> 1. **`region` giant (2718)** — author prerequisite `RegionSurface` (`client/documents/_types`) for `occludedSurfaces`/`define-surface`, then member-diff the ~25 missing live members listed in the phase-7 Tier-B deferral row (`isSingleShape`/`clipperPolyTree`/`area`, `attachment`, `clone`, `clampElevation`, `_createClipperPolyTree`, shape-constraint family, static `_pre*`/`_on*Operation`, `teleportTokens`/`spawnTokens`/`createTokenEmanation`, etc.; drop the stale `_updateTokens`). `Level` + `operators` prereqs already met. Also rides with the **client-data shapes barrel** unification + **`CanvasOcclusionMask#occludedSurfaces`** FIXME.
> 2. **`token` giant (4009)** — author the token-movement subsystem (~15 `TokenMovement*` `_types` + `TokenConstrainMovementPathOptions`), then member-diff; this also unblocks the deferred Token-**placeable** drag/movement surface, the canvas `tokens.d.mts` movement FIXMEs (Batch 5.5b), and `ModifyMovementCost._getTerrainEffects`.
>
> **Then the smaller remaining P7 items:** `scene` final member-order pass; **active-effect final member-order pass** (signatures done, some pre-existing members still in v13 order); Level loose-ends (`PointSourcePolygon.Config.level` + `ClockwiseSweepPolygon` internals); `canvas/vfx/` (~21, deprioritized); MeasuredTemplate reconciliation (the `@deprecated`-class cascade) + **P8 cleanup** (bump `package.json` → `14.x`, prune `removed in v14`).
>
> **Kickoff message** (paste verbatim):
>
> ```
> Continue the v14 migration (Phase 7 — the active-effect and scene Tier-B giants are done; now do the region giant: author RegionSurface in client/documents/_types.d.mts first, then member-diff client/documents/region.d.mts against the v14 source — ~25 missing live members, and the client-data shapes barrel unification rides with it. Then the token giant: author the token-movement _types, then member-diff client/documents/token.d.mts. One giant per session.)
> ```
>
