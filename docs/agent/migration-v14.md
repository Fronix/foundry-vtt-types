...
>
> **Phase 6 is COMPLETE (2026-05-28) — CI green.** All 65 stubs migrated; `src/foundry/client/` is stub-free (zero `@remarks TODO: Stub`). This session also authored the new v14 foundations the stubs needed: `PlaceableConfig` (placeable-config base), the `TokenApplication` mixin members, and the full `DocumentDirectory` sidebar base — plus the two missing journal page sheets and a latent `srcExists` lint fix.
>
> **The only live thread is now Phase 7.** Highest-leverage item: **Scene Levels** — author the `Level` document. It resolves the `object`-typed `FIXME → P7` markers left this session (`scene-config`'s `defaultLevel`/`_getLevelContextOptions`/`_onSortLevel`, and `PlaceableConfig`'s body-level Level coupling) **plus** all of Phase 5's `Level` FIXMEs **plus** unblocks the 4 Tier-B document giants. Then the rest of P7 greenfield (vfx, region-behaviors) and P8 cleanup.
>
> **Kickoff message** (paste verbatim):
>
> ```
> Continue the v14 migration (Phase 7 — Scene Levels: author the Level document, then resolve the object-typed FIXMEs in scene-config/PlaceableConfig + Phase 5)
> ```
>
