...
| **7** | Greenfield: vfx, region-behaviors, **Scene Levels**, 4 Tier-B giants, MeasuredTemplate, shapes barrel | **In progress** — `Level` document authored + fully registered + embedded in Scene (commit `6632620a0`); `SceneLevelsSetField` + `TEXTURE` file category added. CI green. Remaining: per-placeable `levels`/`level` fields, the `object`-typed Level FIXMEs (P5/P6), the 4 giants, vfx, region-behaviors. **Inherits deferrals.** | [phase-7](migration-v14-phase-7.md) |
...
>
> **Phase 7 is IN PROGRESS (2026-05-28) — CI green.** The gating **Level document** is authored and fully wired (commit `6632620a0`): `BaseLevel` (common) + `Level` (client, `ClientDocumentMixin`), full schema, `SceneLevelsSetField`, the v14 `TEXTURE` file category, complete registration across every document surface (constants, documentConfiguration, CONFIG.Level, barrels, globals, eslint, `Document.DropDataFor`), and embedded into the Scene schema as the `levels` collection. A `level.test-d.ts` covers the surface.
>
> **Next, in priority order:**
>
> 1. **Per-placeable Level fields** — now that `SceneLevelsSetField` exists, replace the `// TODO(v14-levels): levels: SceneLevelsSetField (Phase 7)` stubs in the placeable doc schemas (tile, note, drawing, ambient-light, ambient-sound, region, wall, token) with the real `levels`/`level` fields (diff each against its v14 `common/documents/*.mjs`).
> 2. **Tighten the `object`-typed Level FIXMEs** now that `Level` exists: `scene-config` (`defaultLevel`/`_getLevelContextOptions`/`_onSortLevel`), `PlaceableConfig` (P6 deferral), and all the Phase-5 canvas `Level` FIXMEs (edges/sources/perception/groups/Canvas god-object — see the phase-7 Scene-Levels deferral row).
> 3. **The 4 Tier-B giants** (active-effect independent; scene/region/token now have a real `Level` to type against — still need `operators.mjs` + token-movement `_types`).
> 4. Then the rest of P7 greenfield (region-behaviors, vfx) and P8 cleanup.
>
> **Kickoff message** (paste verbatim):
>
> ```
> Continue the v14 migration (Phase 7 — wire the per-placeable levels/level fields now that the Level document exists, then tighten the object-typed Level FIXMEs in scene-config/PlaceableConfig/Phase-5 canvas)
> ```
>
